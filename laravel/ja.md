# 日本語対応の設定
ビューの表示を日本語にする方法を列記する。
## laravelプロジェクトのconfigを設定
`config/app.php`
下の２行を日本標準にするように設定する。
```
'timezone' => 'Asia/Tokyo', //初期設定ではUTC

（中略）

'locale' => 'ja' //初期設定ではenになっている。
```

## langファイルにjaディレクトリを作成
もともとenディレクトリはあるので並列階層にjaディレクトリを作成。（laravel9かららしいので注意）<br />
以下４つのファイルを作成
```
・auth.php
・pagination.php
...
## それぞれのファイルの設定
以下公式ドキュメントに記載している言語ファイルをコピペで問題ないような気がしてる。<br />
[公式ドキュメントURL](https://readouble.com/laravel/9.x/ja/releases.html)
laravel9のドキュメントから以下参考として抜粋する。
### auth.php
```
<?php
...
```
カスタムバリデーションを追加する場合（特定のカラムの日本語対応を実施したい場合など）は`validation.php`の最後から３行目あたりの'attributes'の配列内に記載する。<br />
```:記載例
'attributes' => [
    'name' => '氏名',
    'age' => '年齢',
